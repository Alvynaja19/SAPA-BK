...
<div class="space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1 max-w-2xl">
        Pantau interaksi bimbingan konseling digital siswa bersama Asisten Cerdas AI (Gemini) dan evaluasi respons.
      </p>
    </div>
    ...
    <a
      ...
      class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 text-xs font-bold transition-all shadow-2xs shrink-0 self-start sm:self-auto"
      title="Buka Ruang Konseling Tatap Maya Langsung"
    >
  ...
  <div class="rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-5 shadow-xs">
    <form method="GET" action="{{ route('bk.percakapan') }}" class="space-y-4">
      ...
      <!-- Baris 2: Filter Kelas, Filter Tanggal, dan Tombol Terapkan (3 Kolom Proporsional) -->
        ...
        <div class="p-5 sm:p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4 hover:bg-gray-50/50 dark:hover:bg-gray-800/40 transition-colors">
          ...
          <div class="flex items-start gap-4 min-w-0 flex-1">
            ...
            <div class="space-y-1.5 min-w-0 flex-1">
              ...
              @if($previewMsg)
                <div class="flex items-start gap-2 text-xs text-gray-600 dark:text-gray-300 bg-gray-50/80 dark:bg-gray-800/60 px-3 py-1.5 rounded-lg border border-gray-100 dark:border-gray-800">
                  ...
                  <span class="line-clamp-1 italic text-gray-600 dark:text-gray-300">
                    "{{ Str::limit($previewMsg->content, 110) }}"
                  </span>
                </div>
              @endif

              <!-- Meta Info Waktu & Dialog -->
              <div class="flex items-center gap-2.5 text-[11px] text-gray-400 flex-wrap">
                <span>{{ $s->created_at->format('d M Y, H:i') }} WIB ({{ $s->created_at->diffForHumans() }})</span>
                <span>•</span>
