...
<div class="space-y-6">
  ...
  <div class="rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-5 shadow-xs">
    <form id="liveChatFilterForm" method="GET" action="{{ route('bk.live-chat.riwayat') }}" class="space-y-4">
      
      <!-- Baris 1: Pencarian Siswa / Topik (Live Search Instant) -->
      <div class="space-y-1.5">
        <label for="filterSearchLive" class="block text-xs font-bold text-gray-700 dark:text-gray-300">
          Pencarian Siswa / Topik Konseling (Live Search)
        </label>
        <div
          x-data="{ 
        ...
        >
          ...
          <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center gap-1.5">
            ...
            <button
              ...
              x-show="query"
              x-transition.opacity.duration.150ms
              @click="query = ''; $nextTick(() => { document.getElementById('filterSearchLive').focus(); if(window.performLiveChatLiveSearch) window.performLiveChatLiveSearch(true); })"
    ...
    <div id="liveChatFiltersSummaryContainer">
  ...
  <div id="liveChatSessionsCard" class="rounded-3xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-xs overflow-hidden transition-opacity duration-150">
...
document.addEventListener('DOMContentLoaded', function () {
  const form = document.getElementById('liveChatFilterForm');
  const searchInput = document.getElementById('filterSearchLive');
  const kelasSelect = document.getElementById('filterKelasLive');
  const tanggalInput = document.getElementById('filterTanggalLive');
  const spinner = document.getElementById('liveSearchSpinner');
  const cardContainer = document.getElementById('liveChatSessionsCard');
  const summaryContainer = document.getElementById('liveChatFiltersSummaryContainer');
  ...
  function doFetch(url) {
    ...
    fetch(url, {
      ...
      .then(html => {
        ...
        const newCard = doc.getElementById('liveChatSessionsCard');
        if (newCard && cardContainer) {
          cardContainer.innerHTML = newCard.innerHTML;
        }

        const newSummary = doc.getElementById('liveChatFiltersSummaryContainer');
  ...
  window.performLiveChatLiveSearch = triggerSearch;
